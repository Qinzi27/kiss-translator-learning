...
更新日期：2026-09-16（Australia/Sydney）。基础版本：KISS Translator 2.0.32，提交 `2656f564dde5b271a8d3fb31e951a8457e8df41c`；当前学习版 `2.0.37-learning.8`。
...
## TypeScript 第 1 轮迁移（开发分支）

2026-09-16，基于上面的 learning.8 发布提交。翻译进度与按钮配色迁移为严格 TypeScript，保留既有 JavaScript 测试；新增编译期正反契约和仅诊断构建启用的启动计时，以及离线构建体积工具。原始 MyMemory、网络策略、Argos、PDF 与更新器验证脚本和历史回执未被替换。

完整回归 **183 套 / 2973 项 JavaScript 测试通过**，其中原有 181 套 / 2950 项保留；最终计时编译开关另验 2 套 / 42 项（有重叠，不相加）。类型检查、修改文件 ESLint、Chrome 生产构建，以及离线服务 **29 项**、更新器 **30 项**合成测试均通过。类型检查只涵盖已迁移模块和编译期契约，未声称全仓 JavaScript 已有类型保障。

旧版构建基线与保留的公开安装包逐入口 SHA-256 匹配。新 content.js 为 2,021,435 字节，比旧版多 1,120 字节；后台大小不变。正常 Chrome 构建无计时标记或诊断日志字符串。三个真实本机 Web 开发预览刷新样本中，run 初始化总计为 5.0 / 4.7 / 5.8 ms，首次 FAB layout commit 为 19.8 / 19.4 / 27.4 ms。时间从静态 import 完成后开始，不含下载/解析/注入等待，不是 paint、生产扩展冷启动或迁移提速证据。

方案、命令、边界及后续模块拆分候选见 [迁移记录](docs/MIGRATION.md)，机器可读摘要见 [typescript-phase1.json](validation/typescript-phase1.json)。本轮沿用基础版本元数据进行开发，未发布新的同版本安装包；真实 API、断网模型与 Chrome 安装验收没有因本轮合成回归而重复计为通过。

## learning.8 按钮位置与持续翻译锁定验证

完整 JavaScript 回归 **181 套、2950 项全部通过**。本轮修改的 JavaScript ESLint 零诊断，Chrome 生产构建通过，版本 `2.0.37-learning.8`。没有增加依赖或扩展权限；锁文件未变，旧依赖审计不计作本轮重审。590 个待发布文件的常见凭据格式扫描没有命中，构建排除了 PDF 与开发示例页；格式扫描不保证不存在其他形式的秘密。

- 首次提交测试在布局 effect 中读取真实 DOM transform，确认保存位置直接出现在首帧；覆盖贴边、异常坐标、初始 viewport 尺寸为零及后续 resize。按钮仍随新文档重新注入，不能保证跨文档保留同一个 DOM 节点。
- 使用真实 ContentFab + Draggable 的 DOM / Shadow DOM 组合测试长按 650 毫秒、单次触发、松手抑制单击、下一次正常单击、拖动/提前松手/取消/失焦/卸载取消，以及键盘菜单。正向单测通过受控可信事件桩驱动计时；另行验证真实合成事件 `isTrusted=false` 不得切换锁定，不能把单测等同于物理长按实测。
- 存储及生命周期测试覆盖独立严格布尔锁、FAB 位置和颜色写入不覆盖锁、旧规则不授权新网页、其他标签只同步标记、导航与 BFCache 重新读取、普通锚点保留状态、SPA 新路径、body 替换、黑名单及 iframe/PDF 排除。待保存解锁、另一标签解锁、过期导航读取不能重新启用；新页面手动开启或停止优先于较旧的异步规则读取。写入失败和写入成功但确认失败有不同提示。
- 初始化在设置、规则、词表、FAB 与 iframe 就绪等待期间跳转，会重新校验当前地址及组件黑名单；4 轮内无法稳定则放弃启动。公开网页消息和合成 DOM 输入不能写锁或授权自动翻译。

**真实 Web 预览：** 独立回环开发页使用“仅本机离线”策略，观察默认未锁定；通过键盘菜单开启锁定，小锁与翻译中状态出现。刷新仍为锁定并启动翻译流程，远程服务请求受离线策略阻止；解锁保留当前页状态，随后刷新恢复空闲原文。验证只使用本项目示例文案，没有发送用户 PDF、Key 或登录会话。此轮未新增真实云翻译结果。

**验收边界：** 当前工具不能操作受保护的 Chrome 扩展页面。物理鼠标长按、已安装扩展的跨站导航/多标签存储事件以及原生文件夹授权更新流程仍待用户实机确认；Web 预览和合成回归不能替代这些检查。本版继续按预发布提供。

## learning.7 Chrome 内更新与翻译状态验证

最终完整 JavaScript 回归 **179 套、2834 项全部通过**。本轮改动的生产 JavaScript ESLint 零诊断，Chrome 生产构建通过，数字版本 `2.0.36`、学习版标识 `2.0.36-learning.7`；没有新增依赖或清单权限。锁文件未变，旧依赖审计不计作本轮重审。

- 浏览器更新测试覆盖：开发者模式门控、挂载不联网/不授权、初始化恢复记录读取失败不开放动作、目录句柄权限、纯文本探测清理和错误副本拒绝、下载大小/摘要/清单校验、恶意 ZIP / CRC / 解压限额、碎片流与挂起下载的取消及截止时间、严格 IndexedDB 事务完成、写入前提交备份、最后写 options / manifest、外部编辑保护、配额与各写入阶段故障、取消后恢复及跨标签锁/重新加载保护。
- 新增崩溃快照检查：新版管理路径持久化后、ready 标记前中断，重开必须恢复，恢复后的旧文件与旧路径记录逐项一致。最后 manifest 的 close 尚未完成时取消，也会恢复而不提交 ready。安装测试使用内存文件系统和持久状态快照，不等同于真实磁盘失电测试。
- 翻译按钮使用真实 Translator 与异步 DOM 生命周期的合成测试，覆盖准备、语言检测、排队、API 等待、DOM 完成、服务失败、停止/重开、换服务、新页面与迟到响应；TaskPool 验证 clear 后退避任务不再发送、旧在途结果不交付。PDF 测试覆盖当前页面/配置对应的完成状态、加载与翻译中图标、点击停止。颜色测试覆盖六位色值校验、自动对比色、保存/失败、默认恢复、位置和点击偏好保留、存储变更同步。
- 完整回归首次暴露旧 Translator 测试只等待两个微任务导致 15 项过早断言。测试助手改为真实事件循环检查点，保持原虚拟手势计时和所有功能断言，不改生产翻译逻辑；相关 10 套 313 项及上面的完整回归均通过，数量不相加。

**真实公开附件：** 使用当前 `browserUpdateRelease`、`networkPolicy`、`browserUpdateZip` 模块，在 Node 24 中匿名请求 GitHub API 与公开附件；仅将浏览器设置来源替换为明确的测试策略。先验证 offline 在 fetch 前拒绝，再用 no-google 策略读取当时最新的 `2.0.35-learning.6`，4 次请求 HTTP 200，经发布记录/SHA-256/CRC 校验后解出 267 个 Chrome 文件、13,990,456 字节。没有访问浏览器配置、用户账户或写入安装目录。回执见 [browser-update-live.json](validation/browser-update-live.json)。

```sh
# 可选联网验收：真实公开发布下载与内存解压，不安装；需要 Node 24 和开发依赖
node src/scripts/test-browser-update-live.cjs
# 合成回归（不调用翻译服务）
CI=true node_modules/.bin/react-app-rewired test --watchAll=false --runInBand
pnpm build:chrome
```

**真实 Web 预览：** 在独立回环开发页查看更新面板和概览颜色卡片，显示版本正确，普通网页的目录授权/更新/重新加载按钮全部禁用。将翻译中色值由 `#F9E5A6` 改为 `#6C3EE8`，预览变化且刷新保留；随后恢复默认，其他两色保持原值。该预览未请求翻译 API、未读取用户 PDF，也不是已安装扩展跨标签通信或 Chrome 文件授权验收。

**尚未实机验收：** 当前工具不能操作受保护的 Chrome 扩展页；Chrome 原生文件夹选择、文件权限再次授权、真实安装文件写入与 `runtime.reload` 的完整链路仍待验收。本版按预发布提供。浏览器备份最多一份旧文件内容 64 MiB，不是整目录原子切换；不可据合成回归推断强制断电、外部程序并发修改或所有平台都能无损恢复。步骤及限制见 [UPDATING.md](UPDATING.md)，既有 PDF MIME / Windows / 登录服务边界继续适用。
