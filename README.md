...
当前版本：**`2.0.35-learning.6`（预发布）**。新增无需打开 GitHub 网页的本地一键更新工具，保留 PDF 双语阅读、后两页预翻译、会话缓存和安全修复。使用前请阅读 [PDF 阅读说明](docs/PDF-READER.md)与[安全与升级说明](SECURITY.md)。
...
**一键更新：** macOS 双击安装目录里的 `更新插件.command`，Windows 双击 `更新插件.bat`。需要 Python 3.9+；工具自动下载、校验并原位替换，保留最近一次文件备份，失败时回退。随后在扩展管理页点击「重新加载」，再刷新阅读页。无需登录或打开 GitHub 网页，仍需能连接下载源。详细步骤见 [更新工具说明](UPDATING.md)。通常无需卸载或清空设置；已有设置可能不会自动补入新预设，缺少 MyMemory 时可在向导添加，缺少 Argos 时按[手动添加步骤](offline/README.md#没有-argos-预置时手动添加)配置。商店中的上游版本与本学习版不同。
