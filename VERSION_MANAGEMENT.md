...
本分支基于上游 2.0.32。Chrome 的数字版本使用 `public/manifest.json` 的 `version`，当前为 `2.0.35`；学习版标识使用 `version_name`，当前为 `2.0.35-learning.6`，GitHub 标签对应 `v2.0.35-learning.6`。
...
8. Chrome ZIP 顶层须包含 `更新插件.command`（保留可执行位）、`更新插件.bat`、`updater/update.py` 和 `UPDATING.md`。在临时安装目录运行更新 / 无更新 / 回退验证，再对公开发布运行 `python3 updater/update.py --check`。源码包包含更新器及测试。更新器只替换 `chrome/`，不会自更新外部脚本。
